...
const companyEntries = () => {
  ...
  const walk = (dir) => {
    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (entry.name.endsWith('.html')) entries['company-' + file.slice(root.length + 1).replaceAll('/', '-').replace(/\.html$/, '')] = file;
    }
  };
  walk(resolve(root, 'companies'));
  return entries;
};
