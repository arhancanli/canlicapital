...
- Production checkout: `/Users/arhancanli/canlicapital-production-20260920` (the hourly deploy's design source), detached at 7f8a451e (PR178, v23 activation) since 2026-09-22. Inspect actual deployment/alias state before claiming new production behavior.
- Activation: `config/company-production-activation.json` pins release 4a5d9160…, storage bases and `config/company-admission-v23.json` (option C flag sets). Rebuild admissions with `scripts/build-company-admission.mjs` from the release's discovery, selected-quality report, scope ledgers and `--admit-flags`.
