...
const archivalPaperRoutes = () => {
  ...
  return registry.sleeves.map((paper) => {
    ...
    return {
      ...
      priority: "0.7",
      changefreq: "monthly",
      // No sleeve carries its own generated_at/evidence_date; the registry entry and the two
      // real HTML files that make up the page are what actually change when it does.
      lastmodFiles: [
...
// the sitemap counts what exists rather than what someone remembered to add. Each route carries
// the artifact it renders (`data`) and its resolved raw JSON file (`sourceFile`), reusing the same
// discovery build-measurements.mjs runs, so a page's lastmod is derived from the SAME artifact it
// displays rather than recomputed by a second, potentially-drifting rule.
const measurementRoutes = () => {
  const dir = resolve(ROOT, "measurements");
  if (!existsSync(dir)) return [];
  const researchJsonPath = resolve(ROOT, "public/glassbox/research.json");
  const research = JSON.parse(readFileSync(researchJsonPath, "utf8"));
  const artifacts = discoverMeasurementArtifacts(research);
  const bySlug = new Map(artifacts.map((a) => [slugifyArtifactPath(a.path), a]));
  return readdirSync(dir)
    .filter((name) => name.endsWith(".html"))
    .map((name) => {
      const slug = name.slice(0, -5);
      const artifact = bySlug.get(slug);
      let sourceFile = "public/glassbox/research.json"; // aggregator, last resort
      if (artifact) {
        try {
          sourceFile = `public${rawArtifactUrl(artifact.path, research)}`;
        } catch {
          // No individually-published JSON for this artifact; the aggregator stands in.
        }
      }
      return {
        loc: `${ORIGIN}/measurements/${slug}`,
        priority: "0.7",
        changefreq: "weekly",
        lastmodArtifact: artifact?.data,
        lastmodFiles: [sourceFile],
      };
    });
};

// Slugify has to match build-measurements.mjs's own rule exactly, since these routes are joined
// back to that discovery by slug. Duplicated rather than imported to keep this file's only import
// from build-measurements.mjs limited to the pure discovery functions it already re-exports.
const slugifyArtifactPath = (path) => path.replace(/[._]/g, "-").toLowerCase();
...
const trialRoutes = () => {
  ...
  return index.packets
    ...
    .map((packet) => {
      ...
      const packetFile = `public/glassbox/trial-packets/${packet.hypothesis_key}.json`;
      let packetData = null;
      const packetPath = resolve(ROOT, packetFile);
      if (existsSync(packetPath)) packetData = JSON.parse(readFileSync(packetPath, "utf8"));
      return {
        loc: `${ORIGIN}/trials/${packet.hypothesis_key}`,
        priority: "0.6",
        changefreq: "monthly",
        lastmodArtifact: packetData,
        lastmodFiles: [packetFile],
      };
    });
};

// The engineering notes, DERIVED from what build-notes.mjs put on disk, for the same
// reason the measurement pages are: build-notes decides how many there are. Each note's
// lastmod traces to its own hand-authored markdown source (`notes/<slug>.md`), never to
// the day build-notes.mjs happened to run.
function noteRoutes() {
  ...
  return readdirSync(dir)
    ...
    .map((name) => {
      const slug = name.slice(0, -5);
      return {
        loc: `${ORIGIN}/notes/${slug}`,
        priority: "0.8",
        changefreq: "monthly",
        lastmodFiles: [`notes/${slug}.md`],
      };
    });
}
...
function staticRouteLastmod(path, buildDate) {
  const files = PAGE_SOURCES[path];
  if (!files) {
    throw new Error(`no lastmod source declared for static route "${path}" in PAGE_SOURCES`);
  }
  return resolveLastmod({
    ...
    onFallback: () => {
      BUILD_DATE_FALLBACK_WARNINGS.push(
        `git was unavailable for every source of "${path}"; used the build date as a last resort.`,
      );
    },
  });
}
...
const HUB_ESSAYS = {
  "killed-candidates": [
    "This collection contains the project’s published rejection papers. Each concerns a specified candidate and the evidence available for that decision. A rejection can follow a return test, a cost or capacity constraint, or a failure to obtain the required source evidence. Those outcomes answer different questions and should be read separately.",
    "Start with the candidate’s specification and rejection reason, then inspect the data window, costs, trial accounting and source links. A failed performance test applies to that construction under those assumptions. A source-coverage failure instead means the proposed test could not proceed as specified; it does not establish that the economic mechanism has no effect.",
    "The papers cover equity factors, crypto constructions, positioning, carry and inventory mechanisms tested in this project. Their outcomes do not establish that these research areas are exhausted or that another study must reach the same result. Differences in data, execution assumptions and the selection process need to be examined before comparing results.",
    "Use the linked papers alongside the trial register. The register identifies attempts and missing evidence; the papers explain particular decisions. Keeping rejected candidates visible helps readers inspect the selection process, but a published rejection does not make a surviving candidate profitable or its forward record mature."
  ],
  "literature-reviews": [
...
function main() {
  ...
  // Hub pages are two things at once: the member papers they aggregate, and the hand-written
  // HUB_ESSAYS prose that lives in THIS script. Either can move a hub's lastmod.
  const hubScriptDate = resolveLastmod({ root: ROOT, files: ["scripts/build-papers.mjs"], buildDate });
  const hubLastmods = new Map(
    hubs.map((hub) => {
      const memberDates = papers
        .filter((paper) =>
          KINDS.includes(hub) ? paper.taxonomy.kind === hub : paper.taxonomy.subjects.some((s) => s.slug === hub.slug),
        )
        .map((paper) => paperLastmods.get(paper.slug));
      const latest = [hubScriptDate, ...memberDates].sort().at(-1);
      return [`${ORIGIN}/research/topics/${hub.slug}`, latest];
    }),
  );
  ...
  const urls = [
    ...
    ...STATIC_ROUTES.map((route) => {
      ...
      if (route.path === "/research") {
        lastmod = maxDate([staticRouteLastmod(route.path, buildDate), ...hubLastmods.values(), ...paperLastmods.values()]);
      } else if (route.path === "/measurements") {
        lastmod = maxDate([staticRouteLastmod(route.path, buildDate), ...measurementLastmods.values()]);
      } else if (route.path === "/trials") {
        lastmod = maxDate([staticRouteLastmod(route.path, buildDate), ...trialLastmods.values()]);
      } else if (route.path === "/notes") {
        lastmod = maxDate([staticRouteLastmod(route.path, buildDate), ...noteLastmods.values()]);
      } else {
        lastmod = staticRouteLastmod(route.path, buildDate);
      }
    ...
    ...hubs.map((hub) => ({
      ...
      priority: "0.8",
      changefreq: "weekly",
      lastmod: hubLastmods.get(`${ORIGIN}/research/topics/${hub.slug}`),
    })),
    ...papers.map((paper) => ({
      loc: `${ORIGIN}/research/${paper.slug}`,
      priority: "0.7",
      changefreq: "monthly",
      lastmod: paperLastmods.get(paper.slug),
    })),
    ...archivalPapers.map((route) => ({ ...route, lastmod: archivalLastmods.get(route.loc) })),
    // The measurement, trial and note pages, DERIVED from what their own generators put on disk.
    // Listing them here by hand would reintroduce exactly the defect this file's header describes:
    // a sitemap that stops matching the pages long before anyone notices.
    ...measurements.map((route) => ({ ...route, lastmod: measurementLastmods.get(route.loc) })),
    ...trials.map((route) => ({ ...route, lastmod: trialLastmods.get(route.loc) })),
    ...notes.map((route) => ({ ...route, lastmod: noteLastmods.get(route.loc) })),
  ];

  if (BUILD_DATE_FALLBACK_WARNINGS.length) {
    console.warn(
      `\nlastmod fell back to the build date for ${BUILD_DATE_FALLBACK_WARNINGS.length} source(s) ` +
