...
import { EDITORIAL_OBSERVATION_EXCLUSIONS_V20 } from './lib/company-editorial-v20.mjs';
...
const decisions = EDITORIAL_OBSERVATION_EXCLUSIONS_V20.filter(d => ['0001280452', '0001659494', '0000059255', '0000065596', '0001425205', '0001484612', '0001607962'].includes(d.cik));
...
for (const file of manifest.files) {
  ...
  const after = companyReference(source, { expectedCik: file.cik, fetchedAt: before.fetched_at, selectionPolicy: 'extended-v20' });
  ...
  const expected = structuredClone(before); expected.selection_policy = 'extended-v20';
...
assert.equal(companies, 3323); assert.equal(histories, 87344); assert.equal(changes.length, 30);
const result = {
  schema: 'canli.five-cohort-v20-transition.v1', publication_approved: false,
  ...
  policy_sha256: hash(readFileSync(new URL('./lib/company-editorial-v20.mjs', import.meta.url))),
  companies, histories, explicit_observation_holds: 30, residual_constant_observations_omitted: 6, removed_histories: 2, changes,
  scope: 'Every v14 company source and selected record verified, then reselected under v20. Exact differences are policy metadata, thirty held observations and their notices, and two Valhi share histories omitted because their remaining three observations are constant. Original source bytes remain unchanged. No new delivery/catalog/discovery build, hosted transfer, production or indexing claim.',
};
writeFileSync(process.argv[2], JSON.stringify(result, null, 2) + '\n', { flag: 'wx' });
console.log(JSON.stringify({ companies, histories, explicit_observation_holds: 30, removed_histories: 2 }));
