...
const [catalogDir, deliveryDir, distDir, portText = '4187'] = process.argv.slice(2);
if (!distDir) throw new Error('Usage: node scripts/preview-company-delivery.mjs CATALOG DELIVERY DIST [PORT]');
const port = Number(portText);
if (!Number.isInteger(port) || port < 1024 || port > 65535) throw new Error('Invalid preview port');
const { server } = companyPreviewServer({ catalogDir, deliveryDir, distDir });
