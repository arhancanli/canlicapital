"""Map deferred basic/diluted observations to retained primary filings; preserve gaps."""
import hashlib
import json
...
assert queue['schema'] == 'canli.basic-diluted-review-queue.v1'
assert len(queue['cases']) == queue['groups']
...
report = {'schema': 'canli.equal-history-retained-targets.v1', 'publication_approved': False,
          ...
          'scope': 'All basic/diluted equality cases mapped to exact selected accessions. Missing primary captures remain explicit; retained presence is not numerical or scope verification. No source requests or admission.'}
if len(sys.argv) == 4:
