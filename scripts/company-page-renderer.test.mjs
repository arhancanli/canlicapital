...
test('batch2 Aspira and Sangamo notes preserve allocation, share units and exact reviewed periods', () => {
  for (const [cik, phrases] of [
    ['0000926617', [/no contractual obligation to share losses/, /share counts and per-share amounts are unscaled/]],
    ['0001001233', [/178\.286 million/, /11,000 of non-controlling-interest loss/, /50\.5 million and 96\.5 million/, /inline tags use scale 3/]],
  ]) {
    ...
    assert.equal(notes.length, cik === '0000926617' ? 1 : 2);
...
test('batch2 profitable totals retain continuing-loss dilution and historical split basis', () => {
  for (const [cik, phrases, notesExpected] of [
    ['0000921299', [/positive total EPS of \$45\.37/, /losses from continuing operations/, /one-for-25 reverse split/, /older values have not been rescaled/], 2],
    ['0000924396', [/July 31, 2024 and 2025/, /name change from OpenLocker/], 1],
    ['0000948320', [/preferred dividends/, /unissued vested restricted stock units/, /separately rounded continuing\/discontinued EPS/], 1],
  ]) {
    const raw = gunzipSync(readFileSync(new URL(`./fixtures/editorial/batch2-${cik}-source.json.gz`, import.meta.url)));
    const record = companyReference(raw, { expectedCik: cik, fetchedAt: '2026-09-21T00:00:00Z', selectionPolicy: 'extended-v22' });
    verifyCompanyReference(record, raw);
    ...
    assert.deepEqual(record.concepts, original);
    const notes = companyFilingNotes(record, 'EarningsPerShareBasic');
    assert.equal(notes.length, notesExpected);
    for (const note of notes) {
      const changed = structuredClone(record), row = note.observations[0];
      changed.concepts.find(c => c.tag === row.tag).observations.find(o => o.accn === row.accn && o.end === row.end).val += 1;
      assert(!companyFilingNotes(changed, row.tag).includes(note));
...
test('batch2 notes distinguish limited presentation, dollar scale and included prefunded shares', () => {
  for (const [cik, phrases] of [
    ['0001042418', [/presentation only/, /does not establish a specific reason/]],
    ['0001043961', [/dollar amounts in thousands/, /without multiplying shares by one thousand/]],
    ['0001070050', [/15,000 deemed dividend/, /8\.948 million/, /scale zero/]],
    ['0001074871', [/already include shares issuable under pre-funded warrants/, /one-for-30 reverse split/, /March fiscal dates/]],
  ]) {
    const raw = gunzipSync(readFileSync(new URL(`./fixtures/editorial/batch2-${cik}-source.json.gz`, import.meta.url)));
    const record = companyReference(raw, { expectedCik: cik, fetchedAt: '2026-09-21T00:00:00Z', selectionPolicy: 'extended-v22' });
    verifyCompanyReference(record, raw);
...
test('Plug historical restatement and common-stockholder numerator retain source facts', () => {
  const raw = gunzipSync(readFileSync(new URL('./fixtures/editorial/batch2-0001093691-source.json.gz', import.meta.url)));
  const record = companyReference(raw, { expectedCik: '0001093691', fetchedAt: '2026-09-21T00:00:00Z', selectionPolicy: 'extended-v22' });
  verifyCompanyReference(record, raw);
  const original = structuredClone(record.concepts);
  record.source_snapshot = `/company-data/sources/${record.source_sha256}.json.gz`;
  for (const target of ['overview', 'EarningsPerShareBasic', 'EarningsPerShareDiluted', 'WeightedAverageNumberOfSharesOutstandingBasic', 'WeightedAverageNumberOfDilutedSharesOutstanding']) {
    const html = renderCompanyPages(record, { target })[0].html;
    assert.match(html, /2018 and 2019 figures are explicitly presented as restated/);
    assert.match(html, /after preferred dividends, deemed dividends and discount accretion/);
    assert.match(html, /shares and per-share amounts are unscaled/);
  }
  assert.deepEqual(record.concepts, original);
  const notes = companyFilingNotes(record, 'EarningsPerShareBasic').filter(n => n.observations.some(r => r.accn === '0001558370-22-003577')); assert.equal(notes.length, 1);
  const changed = structuredClone(record), row = notes[0].observations[0];
  changed.concepts.find(c => c.tag === row.tag).observations.find(o => o.accn === row.accn && o.end === row.end).val += 1;
  assert(!companyFilingNotes(changed, row.tag).includes(notes[0]));
});
...
test('Rocket, Dror and ClearPoint retain excluded-share scope and rounded-loss context', () => {
  for (const [cik, accession, phrases] of [
    ['0001281895', '0000950170-25-029002', [/already outstanding for basic EPS/, /exclusion table uses period-end amounts/, /two selected years, not every year/]],
    ['0001282980', '0001213900-26-021201', [/Series A preferred share as convertible into 100 common shares/, /zero EPS is a rounded \$2,544,887 net loss/]],
    ['0001285550', '0001193125-26-111231', [/approximately 3 million excluded/, /approximate excluded-instrument amount/]],
  ]) {
    const raw = gunzipSync(readFileSync(new URL(`./fixtures/editorial/batch2-${cik}-source.json.gz`, import.meta.url)));
    const record = companyReference(raw, { expectedCik: cik, fetchedAt: '2026-09-21T00:00:00Z', selectionPolicy: 'extended-v22' });
    verifyCompanyReference(record, raw);
...
test('CNBX and TurnOnGreen retain fiscal dates, rounded loss and preferred-dividend context', () => {
  for (const [cik, phrases] of [
    ['0001343009', [/August 31, 2024 and 2025, not calendar years/, /loss EPS as \(0\.00\)/, /does not mean the company had no loss/, /anti-dilutive in both periods/]],
    ['0001349706', [/period-end potential shares are not additions/, /waiving accrued and future preferred dividends/, /125% liquidation preference/, /additional paid-in capital/]],
  ]) {
    const raw = gunzipSync(readFileSync(new URL(`./fixtures/editorial/batch2-${cik}-source.json.gz`, import.meta.url)));
    const record = companyReference(raw, { expectedCik: cik, fetchedAt: '2026-09-21T00:00:00Z', selectionPolicy: 'extended-v22' });
    verifyCompanyReference(record, raw);
...
test('Sonendo Bespoke and Cannabis Bioscience preserve operations fiscal years and small EPS', () => {
  for (const [cik, phrases] of [
    ...
    ['0001409197', [/2018 and 2019 fiscal years ended August 31/, /2018 net loss and a 2019 net profit/, /does not establish the exclusion reason for 2023/, /45-to-1 reverse split effective January 13, 2023/, /separate from the later December year-end history/]],
    ['0001411057', [/fiscal years end May 31/, /EPS of -\$0\.00006 and -\$0\.00005/, /not zero income/, /does not establish that no potentially convertible instruments existed/]],
  ]) {
    const raw = gunzipSync(readFileSync(new URL(`./fixtures/editorial/batch2-${cik}-source.json.gz`, import.meta.url)));
    const record = companyReference(raw, { expectedCik: cik, fetchedAt: '2026-09-21T00:00:00Z', selectionPolicy: 'extended-v22' });
    verifyCompanyReference(record, raw);
    ...
    assert.deepEqual(record.concepts, original);
    const notes = companyFilingNotes(record, 'EarningsPerShareBasic');
    assert.equal(notes.length, cik === '0001409197' ? 2 : 1);
    for (const note of notes) {
      const changed = structuredClone(record), row = note.observations[0];
      changed.concepts.find(c => c.tag === row.tag).observations.find(o => o.accn === row.accn && o.end === row.end).val += 1;
      const remaining = companyFilingNotes(changed, row.tag);
...
test('Fate, Range, Marvion and Biodesix preserve filing-specific EPS context', () => {
  for (const [cik, count, phrases] of [
    ['0001434316', 2, [/selected 2020 and 2021/, /118,789,974 denominator/, /113,685,177/, /nominal-exercise-price prefunded warrants/]],
    ['0001438943', 1, [/net income in 2025/, /does not independently establish the exclusion calculation/, /not additional weighted-average shares/]],
    ['0001439264', 1, [/numeric zero does not mean break-even/, /parenthesized EPS/, /does not establish why potential shares/]],
    ['0001439725', 1, [/6,470,000 and 7,551,000/, /one-for-20 reverse stock split/, /September 15, 2025/]],
  ]) {
    const raw = gunzipSync(readFileSync(new URL(`./fixtures/editorial/batch2-${cik}-source.json.gz`, import.meta.url)));
    const record = companyReference(raw, { expectedCik: cik, fetchedAt: '2026-09-21T00:00:00Z', selectionPolicy: 'extended-v22' });
    verifyCompanyReference(record, raw);
