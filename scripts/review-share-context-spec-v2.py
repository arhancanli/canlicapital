...
def main():
    ...
    for decision in review_spec['decisions']:
        ...
        soup = BeautifulSoup(raw, 'html.parser'); table = soup.find_all('table')[decision['table_index']]
        table_text = clean(table)
        assert all(needle in table_text for needle in decision['table_requires'])
        blocks = [clean(n) for n in soup.find_all(['p', 'div', 'td']) if n.find(['p', 'div', 'td']) is None]
        disclosures = []
