...
def paired_hidden(soup, table, check, helper):
    """Explicit Bio-Path mapping: linked EPS, or disclosed shared loss denominator."""
    ...
    for match in check['matches']:
        ...
        peers = [n for n in table.find_all(attrs={'name': 'us-gaap:' + basic})
                 ...
                 and n.get('scale', '0') == ('3' if tag == 'WeightedAverageNumberOfDilutedSharesOutstanding' else '0') and helper.numeric(n) == helper.numeric(node) == Decimal(str(check['selected']['val']))]
...
def main():
    ...
    assert d['cik'] == '0001133818' and d['accession'] == '0001558370-25-003837'
    assert d['primary_sha256'] == '61aa4b3fccbf00c969a8bdaafd57663d39fadf3044a1d0e8cffb02f5d515a9b9'
    assert d['source_sha256'] == 'ba1a5b94a703d7df40927e88576df9dbcb07b38680ce9a942065f8bd6fea9eee'
    assert d['table_index'] == 345 and d['observations'] == 8
    decisions = []
    for decision in review_spec['decisions']:
        ...
        assert any('Consequently, diluted net loss per share as presented in the financial statements is equal to basic net' in text for text in disclosures)
        assert any('when a loss exists, as it would be anti-dilutive' in text for text in disclosures)
        facts = []
            ...
            if not main:
                ...
                facts.append(dict(selected=check['selected'], fact_xml=str(node),
                    ...
                    mapping=('Explicit SEC hidden link in same statement row as visible basic EPS' if links else
                             'Hidden full-share value (scale zero) equals visible basic denominator scaled by 1000, with identical unit and period context, with filing disclosure that basic and diluted loss EPS are the same; no direct hidden DOM link')))
                continue
            for node in main:
