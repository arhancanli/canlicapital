...
const decisions = EDITORIAL_OBSERVATION_EXCLUSIONS_V18.filter(d => ['0001280452', '0001659494', '0000059255', '0000065596', '0001425205'].includes(d.cik));
...
for (const file of manifest.files) {
  ...
  const after = companyReference(source, { expectedCik: file.cik, fetchedAt: before.fetched_at, selectionPolicy: 'extended-v18' });
  ...
  const expected = structuredClone(before); expected.selection_policy = 'extended-v18';
...
assert.equal(companies, 3323); assert.equal(histories, 87344); assert.equal(changes.length, 18);
const result = {
  schema: 'canli.five-cohort-v18-transition.v1', publication_approved: false,
