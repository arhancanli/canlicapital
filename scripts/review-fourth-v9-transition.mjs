...
import {EDITORIAL_EXCLUSIONS_V9} from './lib/company-editorial-v9.mjs';
const hash=b=>createHash('sha256').update(b).digest('hex');
const root='artifacts/seo/corpus-local/fourth-1000-delivery-';
const beforeBytes=readFileSync(root+'v6/delivery.json'),afterBytes=readFileSync(root+'v9/delivery.json');
const before=JSON.parse(beforeBytes),after=JSON.parse(afterBytes);
assert.equal(before.selection_policy,'extended-v6');assert.equal(after.selection_policy,'extended-v9');
...
const additions=EDITORIAL_EXCLUSIONS_V9.filter(d=>before.files.some(f=>f.cik===d.cik)&&!EDITORIAL_EXCLUSIONS_V6.some(o=>o.cik===d.cik&&o.tag===d.tag));
...
for(let i=0;i<before.files.length;i++){
 ...
 const old=record(a,'v6'),next=record(b,'v9'),expected=structuredClone(old);
 expected.selection_policy='extended-v9';previousHistories+=old.concepts.length;currentHistories+=next.concepts.length;
...
const report={schema:'canli.company-policy-transition-review.v1',publication_approved:false,
 ...
 scope:'Exact local v6-to-v9 cohort transition only; not editorial approval of remaining histories or an indexing claim.',
 code_sha256:hash(readFileSync(new URL(import.meta.url)))};
writeFileSync('artifacts/seo/company-fourth-v9-cohort-transition.json',JSON.stringify(report,null,2)+'\n');
