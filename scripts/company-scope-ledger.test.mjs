...
test('registered corpus withdraws only six Lifeward currency-conflict rows and produces deterministic gzip', () => {
  ...
  try {
    ...
    const frozen = JSON.parse(gunzipSync(readFileSync('artifacts/seo/company-basic-diluted-registered-scope-batch35-20260921.json.gz')));
    ...
    for (let i = 0; i < frozen.rows.length; i++) {
      ...
      if (before.cik === '0001607962' && before.observation.unit === 'ILS/shares') {
        assert.equal(before.state, 'ACCOUNTING_SCOPE_REVIEW_PENDING');
        assert.deepEqual(after, { ...before, state: 'WITHDRAWN_BY_V20_OBSERVATION_HOLD', evidence: 'https://www.sec.gov/Archives/edgar/data/1607962/000117891324000730/zk2431037.htm' });
        changed++;
      } else assert.deepEqual(after, before);
    }
    assert.equal(changed, 6);
    assert.equal(current.active_reviewed_observations, 962);
    assert.equal(current.active_observations_needing_scope_review, 188);
