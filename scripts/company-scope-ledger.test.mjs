...
test('registered corpus changes only the 48 newly reviewed batch6 rows and produces deterministic gzip', () => {
  ...
  try {
    ...
    const frozen = JSON.parse(gunzipSync(readFileSync('artifacts/seo/company-basic-diluted-registered-scope-batch5-20260920.json.gz')));
    assert.equal(current.rows.length, frozen.rows.length);
    const newCiks = new Set(['0001125345', '0001162194', '0001163609', '0001227636']);
    let changed = 0;
    for (let i = 0; i < frozen.rows.length; i++) {
      ...
      if (newCiks.has(before.cik)) {
        ...
        assert.deepEqual(after, { ...before, state: 'SCOPE_REVIEWED_WITH_SOURCE_CONTEXT', evidence: 'company-share-context-batch6-20260920.json' });
        changed++;
      } else assert.deepEqual(after, before);
    }
    assert.equal(changed, 48);
    assert.equal(current.active_reviewed_observations, 274);
    assert.equal(current.active_observations_needing_scope_review, 894);
