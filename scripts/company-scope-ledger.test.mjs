...
test('registered corpus changes only the 12 Gladstone presentation review rows and produces deterministic gzip', () => {
  ...
  try {
    ...
    const frozen = JSON.parse(gunzipSync(readFileSync('artifacts/seo/company-basic-diluted-registered-scope-v19-20260921.json.gz')));
    ...
    for (let i = 0; i < frozen.rows.length; i++) {
      ...
      if (before.cik === '0001495240') {
        assert.equal(before.state, 'ACCOUNTING_SCOPE_REVIEW_PENDING');
        assert.deepEqual(after, { ...before, state: 'REPORTED_PRESENTATION_REVIEWED_CAUSE_NOT_ESTABLISHED', evidence: 'company-share-context-gladstone-20260921.json' });
        changed++;
      } else assert.deepEqual(after, before);
    }
    assert.equal(changed, 12);
    assert.equal(current.active_reviewed_observations, 600);
    assert.equal(current.active_observations_needing_scope_review, 556);
