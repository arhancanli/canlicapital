...
test('registered corpus advances only four historical ReWalk review rows and produces deterministic gzip', () => {
  ...
  try {
    ...
    const frozen = JSON.parse(gunzipSync(readFileSync('artifacts/seo/company-basic-diluted-registered-scope-held-eps-20260921.json.gz')));
    ...
    for (let i = 0; i < frozen.rows.length; i++) {
      ...
      if (before.cik === '0001607962' && before.observation.accn === '0001178913-24-000730' && before.observation.unit !== 'ILS/shares') {
        assert.equal(before.state, 'ACCOUNTING_SCOPE_REVIEW_PENDING');
        assert.deepEqual(after, { ...before, state: 'SCOPE_REVIEWED_WITH_SOURCE_CONTEXT', evidence: 'company-rewalk-historical-context-20260921.json' });
        changed++;
      } else assert.deepEqual(after, before);
    }
    assert.equal(changed, 4);
    assert.equal(current.active_reviewed_observations, 1092);
    assert.equal(current.active_observations_needing_scope_review, 58);
