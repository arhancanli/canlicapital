"""Independently compare complete sitemap URL sets and storage plans across v9/v10."""
import hashlib
import json
...
def main():
    old, before, old_sha, _ = inventory('v9')
    new, after, new_sha, shards = inventory('v10')
    removed = sorted(before - after)
    if removed != ['https://canlicapital.com/companies/0001873213/SellingGeneralAndAdministrativeExpense'] or after - before:
        raise ValueError('Unexpected URL changes')
    plan_raw = (C / 'company-five-cohort-storage-plan-v10.json').read_bytes()
    plan = json.loads(plan_raw)
    previous = json.loads((C / 'company-five-cohort-storage-plan-v9.json').read_bytes())
    ...
    result = {'schema': 'canli.five-cohort-v10-discovery-audit.v1', 'publication_approved': False,
              ...
              'storage_objects': plan['objects'], 'storage_bytes': plan['bytes'], 'shared_v9_object_keys': len(common),
    ...
    with (A / 'company-five-cohort-v10-discovery-audit-20260920.json').open('x') as stream:
