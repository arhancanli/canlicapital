...
export function companyReference(raw, { fetchedAt, expectedCik, diagnostics = {}, selectionPolicy }) {
  ...
  if (selectionPolicy !== undefined && ![EXTENDED_POLICY, EDITORIAL_POLICY, EDITORIAL_POLICY_V3, EDITORIAL_POLICY_V4, EDITORIAL_POLICY_V5, EDITORIAL_POLICY_V6].includes(selectionPolicy)) throw new Error('Unknown company selection policy');
  ...
  const decisions = selectionPolicy === EDITORIAL_POLICY_V6 ? EDITORIAL_EXCLUSIONS_V6 : selectionPolicy === EDITORIAL_POLICY_V5 ? EDITORIAL_EXCLUSIONS_V5 : selectionPolicy === EDITORIAL_POLICY_V4 ? EDITORIAL_EXCLUSIONS_V4 : selectionPolicy === EDITORIAL_POLICY_V3 ? EDITORIAL_EXCLUSIONS_V3 : selectionPolicy === EDITORIAL_POLICY ? EDITORIAL_EXCLUSIONS : [];
  const sourceHash = createHash('sha256').update(raw).digest('hex');
  const definitions = [EXTENDED_POLICY, EDITORIAL_POLICY, EDITORIAL_POLICY_V3, EDITORIAL_POLICY_V4, EDITORIAL_POLICY_V5, EDITORIAL_POLICY_V6].includes(selectionPolicy) ? { ...CONCEPTS, ...EXTENDED_CONCEPTS } : CONCEPTS;
