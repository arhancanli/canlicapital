  ...
  for (const filing of primary.filings) {
    ...
    for (const check of filing.checks) {
      ...
      if (matches.length) {
        ...
        assert(['WITHDRAWN_BY_V17_OBSERVATION_HOLD', 'WITHDRAWN_BY_V18_OBSERVATION_HOLD', 'WITHDRAWN_BY_V19_OBSERVATION_HOLD', 'WITHDRAWN_BY_V20_OBSERVATION_HOLD'].includes(state), 'Unsupported hold state');
