...
// filingLink(accession): this site's filing page for a cited accession, or null to cite sec.gov
// only. latestFilings: up to five { accession, form, filed } the overview lists, newest first.
export function renderCompanyPages(company, { target = 'all', filings = null, linkConcept = () => true, filingLink = () => null, latestFilings = [], robots = 'index, follow' } = {}) {
  ...
  const filingsSection = filings ? `<section aria-labelledby="filings"><h2 id="filings">Filings</h2>${latestFilings.length ? `<ul class="company-reference__latest-filings">${latestFilings.map(f => `<li><a href="${pathFor(company)}/filings/${esc(f.accession)}">${esc(f.form)} filed ${esc(f.filed)}</a></li>`).join('')}</ul>` : ''}<p><a href="${pathFor(company)}/filings">${filings.filings} ${esc(label(company))} filings with published measures</a>: what each annual or quarterly report tagged, with the periods it covered, as reported in that filing. The histories above show the latest-filed value per period.</p></section>` : '';
  ...
  for (const concept of company.concepts) {
    ...
    const title = `${titleLabel(company)}: ${concept.label}`;
