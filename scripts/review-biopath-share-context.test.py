...
def load(name, file):
    ...
    return module
review = load('review', 'review-biopath-share-context.py')
...
class HiddenPairTests(unittest.TestCase):
    def fixture(self, shares=False):
        ...
        val, unit, sign = ('478000', 'shares', '') if shares else ('0.39', 'USDshares', ' sign="-"')
        ...
        check = dict(selected=dict(tag=diluted, val=478000 if shares else -0.39), matches=[dict(fact_id='hidden')])
        if shares:
            peer = soup.find(id='basic'); peer['scale'] = '3'; peer.string = '478'
        return soup, check
    ...
    def test_wrong_share_display_or_double_scaling_fails(self):
        for mutation in ['missing_scale', 'wrong_display', 'double_scale']:
            soup, check = self.fixture(True)
            if mutation == 'missing_scale': soup.find(id='basic')['scale'] = '0'
            if mutation == 'wrong_display': soup.find(id='basic').string = '478000'
            if mutation == 'double_scale': soup.find(id='hidden')['scale'] = '3'
            with self.subTest(mutation=mutation), self.assertRaises(AssertionError):
                review.paired_hidden(soup, soup.table, check, helper)
