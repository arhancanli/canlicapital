...
const decisions = EDITORIAL_OBSERVATION_EXCLUSIONS_V19.filter(d => ['0001280452', '0001659494', '0000059255', '0000065596', '0001425205', '0001484612'].includes(d.cik));
...
for (const file of manifest.files) {
  ...
  const after = companyReference(source, { expectedCik: file.cik, fetchedAt: before.fetched_at, selectionPolicy: 'extended-v19' });
  ...
  const expected = structuredClone(before); expected.selection_policy = 'extended-v19';
...
assert.equal(companies, 3323); assert.equal(histories, 87344); assert.equal(changes.length, 24);
const result = {
  schema: 'canli.five-cohort-v19-transition.v1', publication_approved: false,
