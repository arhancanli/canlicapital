...
def paired_hidden(soup, table, check, comparison, cik, helper):
    """Source-pinned MaxCyte mapping: linked EPS, disclosed shared loss denominator."""
    ...
    for match in check['matches']:
        ...
        for link in table.find_all(style=True):
            ...
            if (any(key.strip() == '-sec-ix-hidden' and value.strip() == node['id']
                ...
                links.append(link)
        if check['selected']['tag'] == 'EarningsPerShareDiluted':
            assert len(links) == 1, 'Missing or ambiguous exact hidden link in combined row'
        else:
            assert not links, 'Unexpected hidden-share link; review mapping again'
...
def main():
    ...
    pins = {'0001104659-26-034557': ('9172489f042157874b1ca97f56d9997115d2d0b9c56708f8093be19b9b67f9ef', 'bf885643e4ac829812a590e85ffccacb74e5a1dbcbb30dc2694b3cf04e7ecdf9', 232), '0001558370-22-004081': ('61724714768d887fe94ec09cc7aee42ec5529d78c7cbb89a4efec92004e5fbc2', 'bf885643e4ac829812a590e85ffccacb74e5a1dbcbb30dc2694b3cf04e7ecdf9', 244)}
    assert len(review_spec['decisions']) == 2
    assert {d['accession'] for d in review_spec['decisions']} == set(pins)
    for d in review_spec['decisions']:
        assert d['cik'] == '0001287098' and d['observations'] == 8
        assert (d['primary_sha256'], d['source_sha256'], d['table_index']) == pins[d['accession']]
    decisions = []
    for decision in review_spec['decisions']:
        ...
        assert any('For periods of net loss, diluted loss per share is calculated similarly to basic loss per share because the impact of all dilutive potential common shares is anti-dilutive.' in text for text in disclosures)
        assert 'basic and diluted' in table_text.lower()
        facts = []
        for check in comparison['checks']:
            ...
            if not main:
                assert accession == '0001558370-22-004081', 'Hidden mapping is only reviewed for older filing'
                ...
                facts.append(dict(selected=check['selected'], fact_xml=str(node),
                    ...
                    mapping=('Explicit SEC hidden link in same row as visible basic EPS' if links else 'No direct hidden-share DOM link: identical context, unit and full value to visible basic denominator, with combined basic/diluted row and explicit loss-period dilution disclosure')))
                continue
            for node in main:
