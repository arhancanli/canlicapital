...
for (const filing of primary.filings) {
  for (const check of filing.checks) {
    ...
    const holds = EDITORIAL_OBSERVATION_EXCLUSIONS_V16.filter(d => d.cik === filing.cik && d.tag === observation.tag && Object.entries(d.observation).every(([k, v]) => observation[k] === v));
    ...
    if (holds.length) {
      ...
      assert(!scope.has(id)); state = 'WITHDRAWN_BY_V16_OBSERVATION_HOLD'; evidence = holds[0].filing_url; withdrawn++;
    } else if (scope.has(id)) {
      assert.deepEqual(observation, scope.get(id));
      state = 'SCOPE_REVIEWED_WITH_SOURCE_CONTEXT'; evidence = 'company-111-dilution-context-20260920.json'; activeReviewed++;
    } else pending++;
...
assert.equal(activeReviewed, 14); assert.equal(withdrawn, 6); assert.equal(pending, 1156);
const result = {
  schema: 'canli.basic-diluted-batch1-scope.v1', publication_approved: false,
  ...
  rows,
  scope: 'Exact batch1 partition only. Fourteen 111, Inc. observations reviewed for the stated accounting context, six Valhi rows held by v16, all other observations pending. Other Valhi holds and the separate baseline are outside this batch. Numerical closure does not grant scope approval; no whole-history, issuer, corpus or production admission.',
};
