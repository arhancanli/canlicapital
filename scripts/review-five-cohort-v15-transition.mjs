...
import { EDITORIAL_OBSERVATION_EXCLUSIONS_V15 } from './lib/company-editorial-v15.mjs';
...
const decisions = EDITORIAL_OBSERVATION_EXCLUSIONS_V15.filter(d => ['0001280452', '0001659494'].includes(d.cik));
...
for (const file of manifest.files) {
  ...
  const after = companyReference(source, { expectedCik: file.cik, fetchedAt: before.fetched_at, selectionPolicy: 'extended-v15' });
  ...
  const expected = structuredClone(before); expected.selection_policy = 'extended-v15';
...
assert.equal(companies, 3323); assert.equal(histories, 87346); assert.equal(changes.length, 2);
const result = {
  schema: 'canli.five-cohort-v15-transition.v1', publication_approved: false,
  ...
  policy_sha256: hash(readFileSync(new URL('./lib/company-editorial-v15.mjs', import.meta.url))),
  companies, histories, removed_observations: 2, changes,
  scope: 'Every v14 company source and selected record verified, then reselected under v15. Exact differences are policy metadata, two held observations and their notices. No new delivery/catalog/discovery build, hosted transfer, production or indexing claim.',
};
writeFileSync(process.argv[2], JSON.stringify(result, null, 2) + '\n', { flag: 'wx' });
console.log(JSON.stringify({ companies, histories, removed_observations: 2 }));
