...
say(`- [Tools](${ORIGIN}/tools): Every interactive calculator in one index, source-bound to the same code the site's own numbers run through.`);
say();
say("## Open source");
say();
...
say(`- [MCP server](https://www.npmjs.com/package/canli-validation-mcp): \`npx -y canli-validation-mcp\`. Wraps the same validation API as callable tools for a coding agent: issue a free key, validate a Sharpe ratio or a backtest, fetch a receipt, check service status.`);
say(`- [GitHub Action](https://github.com/arhancanli/validate-backtest-action): \`uses: arhancanli/validate-backtest-action@v0\`. Runs the same validation API against a submitted return series inside CI, so a failing check can gate a merge.`);
say();
...
say(`- [Probability of backtest overfitting calculator](${ORIGIN}/tools/backtest-overfitting): Run Combinatorially Symmetric Cross-Validation on your own matrix of variant returns, the same core the validation API imports.`);
