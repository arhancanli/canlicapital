// Staged-inventory verification for the thirteen-cohort v26 release: the nine v24
// cohorts carried unchanged plus four historical cohorts (policy extended-v23,
// captured 2026-09-22 from the historical bulk discovery), with the filing family.
//
// Checks: every v24 company is carried byte for byte (same source hash, same
...
//   [CORPUS_LOCAL=<corpus-local directory>] node scripts/verify-v26-runtime.mjs OUTPUT.json
...
const catalogDir = base + 'company-thirteen-cohort-catalog-v26';
const deliveryDir = base + 'company-thirteen-cohort-delivery-v26';
const filingsDir = base + 'company-thirteen-cohort-filings-v26';
const discoveryDir = base + 'company-thirteen-cohort-discovery-v26';
const planPath = base + 'company-thirteen-cohort-storage-plan-v26.json';
const previousDir = base + 'company-nine-cohort-delivery-v24';
const PREVIOUS_MANIFEST_SHA256 = 'c1471479c58572ab1193e1aba80ee4e4716de5c70c4a122dc3fc983eba5c9d3b';
const NEW_QUEUES = ['eleventh-1000', 'twelfth-1000', 'thirteenth-1000', 'fourteenth-1000'];
...
for (const cik of newQueue.keys()) assert(!old.has(cik), `queued CIK ${cik} already in v24`);
...
assert.equal(carried, old.size, 'every v24 company must be carried over');
assert.equal(carriedHistories, previous.files.length ? 296819 : 0);
...
const previousPlan = read(base + 'company-nine-cohort-storage-plan-v25.json');
...
const output = {
  schema: 'canli.v26-runtime-verification.v1', publication_approved: false,
  ...
  objects: plan.objects, bytes: plan.bytes, objects_in_v25_plan: carriedObjects, new_objects: plan.objects - carriedObjects,
  ...
  scope: 'Every v24 company carried unchanged; every new company from a historical capture queue; every filings document re-derived and compared; discovery equals the derived URL set; the plan binds the release. Staged inventory only, not editorial admission, hosting or indexing evidence.',
};
