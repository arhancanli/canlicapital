"""Select missing primary filings by observation coverage, bound to current sources."""
import hashlib
import json
...
def main():
    ...
    directory = A / 'corpus-local/company-five-cohort-delivery-v14'
    manifest_raw = (directory / 'delivery.json').read_bytes()
    assert sha(manifest_raw) == '7f2540f7d8393615efff856479b41b5994aac1d258d0e4de1d116ca616c76472'
    ...
    for filing in candidates:
        ...
        retained = list((A / 'corpus-local').glob('**/' + name))
        if retained:
        ...
        if len(selected) == limit:
            break
    result = dict(schema='canli.basic-diluted-capture-batch.v1', publication_approved=False,
        ...
        scope='Bounded primary acquisition batch ranked by missing-observation coverage, then CIK/accession. Exact observations still present in current v14 records; existing captures skipped. Acquisition is not numerical or accounting-scope approval. No source alteration or publication.')
