...
// CIK -> the company's first ticker in the SEC file's order, which lists a company's main share
// class first (GOOGL before GOOG, BRK-B before BRK-A). Page titles read it from this generated
// module so a serverless function bundles it with the renderer.
export const PRIMARY_MODULE = "scripts/lib/company-primary-tickers.mjs";

export function primaryTickers({ secBytes, admission }) {
  ...
  for (const row of rows) {
    ...
    if (!admission.companies[cik] || excluded[cik] || !/^[A-Z0-9.\-]{1,10}$/.test(ticker)) continue;
    if (!(cik in primary)) primary[cik] = ticker;
  }
  return Object.fromEntries(Object.entries(primary).sort(([a], [b]) => (a < b ? -1 : a > b ? 1 : 0)));
}

export function primaryTickerModule(primary, source) {
  return `// Generated by scripts/build-company-tickers.mjs from ${source.url}\n` +
    `// (captured ${source.captured}, sha256 ${source.sha256}). Do not edit by hand.\n` +
    `export const PRIMARY_TICKERS = Object.freeze(${JSON.stringify(primary)});\n`;
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const primary = primaryTickers({ secBytes: readFileSync(secPath), admission });
  writeFileSync(resolve(ROOT, PRIMARY_MODULE), primaryTickerModule(primary, index.source));
  console.log(`${OUTPUT}: ${index.count} tickers from ${index.source.entries} SEC rows; ${PRIMARY_MODULE}: ${Object.keys(primary).length} companies`);
}
