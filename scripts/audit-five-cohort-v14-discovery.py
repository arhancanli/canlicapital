"""Independently compare complete sitemap URL sets and storage plans across v11/v14."""
import hashlib
import json
...
def main():
    old, before, old_sha, _ = inventory('v11')
    new, after, new_sha, shards = inventory('v14')
    removed = sorted(before - after)
    if removed != ['https://canlicapital.com/companies/0001342916/OperatingIncomeLoss'] or after - before:
        raise ValueError('Unexpected URL changes')
    plan_raw = (C / 'company-five-cohort-storage-plan-v14.json').read_bytes()
    plan = json.loads(plan_raw)
    previous = json.loads((C / 'company-five-cohort-storage-plan-v11.json').read_bytes())
    ...
    if (new['companies'], new['histories'], new['directory_pages'], len(after)) != (3323, 87346, 67, 90736):
    ...
    result = {'schema': 'canli.five-cohort-v14-discovery-audit.v1', 'publication_approved': False,
              ...
              'storage_objects': plan['objects'], 'storage_bytes': plan['bytes'], 'shared_v11_object_keys': len(common),
    ...
    with (A / 'company-five-cohort-v14-discovery-audit-20260920.json').open('x') as stream:
