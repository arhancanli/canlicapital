...
export function createReceiptHandler({ store } = {}) {
  return async function handler(req, res) {
    ...
    let row;
    try { row = await (store ?? defaultStore()).getReceipt(id); } catch (e) { console.error("[validation-api] receipt read failed", e.status ?? "", e.message); return send(res, 503, errorEnvelope({ endpoint: "receipts", code: "store_unavailable", message: "The receipt store is unavailable", limits: LIMITS_TEXT })); }
